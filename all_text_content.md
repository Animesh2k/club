# Stitch the Digital Altar System - Complete Website Text Copy

> [!NOTE]
> This file contains structured textual copy extracted from all HTML templates. Use this document as the single source of truth for copywriting, translations, and asset audits.

## 📂 PROJECT FOLDER STRUCTURE & MAP
Below is the developer roadmap mapping the file hierarchy of the codebase:

```
stitch_the_digital_altar_system/
├── assets/
│   ├── css/
│   │   └── main.css             # Main styling, custom Google UI animations, and design tokens
│   ├── js/
│   │   ├── altar-effects.js     # Particle system (altar sparks) and 3D parallax tilt handler
│   │   ├── lightbox.js          # Photo gallery lightbox overlay modal logic
│   │   ├── mobile-menu.js       # Glassmorphic Material Design navigation menu script
│   │   ├── slideshow.js         # Cinematic background slideshow cycler
│   │   └── tailwind-config.js   # Tailored Tailwind configurations
│   └── images/                  # Gallery images, logo, and Goddess Durga silhouette
├── legal/
│   └── privacy_policy.html      # Privacy policy page
├── programs/
│   ├── anno_seva.html           # Annadan (Food Seva) information and gallery
│   └── cultural_program.html    # Cultural performance events and media grid
├── services/
│   ├── ambulance_contact.html   # Rapid response medical ambulance page
│   ├── blood_donor.html         # Verified blood donor registration and lookup
│   └── emergency_services.html  # Directory of medical support programs
├── index.html                   # Mobile-first interactive home landing page
├── about.html                   # Organizational heritage, history, and vision
├── committee.html               # Board members directory and portfolios
├── gallery.html                 # Comprehensive festive media grid
├── contact_us.html              # Dynamic contact page
└── all_text_content.md          # Complete project textual copy database (this file)
```

## 📄 WEBSITE CONTENT DATABASE BY PAGE
HTML layouts are parsed below. Boilerplate tags, scripts, styles, navigation bars, and footers are omitted for clarity.

---

## PAGE: INDEX.HTML
- 📄 **Page Title**: `Sharad Sangha - Home`
- 🔗 **Source File**: `[index.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/index.html)`

### Headings
- **H1**: `শারদ সংঘ`
- **H1**: `শারদ সংঘ`
- **H2**: `Celebrating Durga Puja for the past eight decades`
- **H2**: `Community & Social Services`
- **H3**: `Annual Blood Donation Camp`
- **H4**: `Emergency Ambulance`
- **H4**: `Cultural Programs`
- **H4**: `Anno Seva`
- **H2**: `5K+ Puja Premi and Pujo Paglos's`
- **H2**: `Visit the Altar`
- **H3**: `Byabattarhat`

### Content & Paragraphs
- `সৃজনে উদ্ভাবনে আর এক আবেগময় পথচলা -`
- `Byabattarhat Sharad Sangha`
- `Est. 1948`
- `সৃজনে উদ্ভাবনে আর এক আবেগময় পথচলা -`
- `Byabattarhat Sharad Sangha`
- `Est. 1948`
- `দুর্গা অন্ত প্রাণ....`
- `For eighty years, our sangha has been the beating heart of Byabattarhat's cultural identity, weaving together tradition and modern artistic expression.`
- `Our Commitment`
- `In times of crisis, every drop matters. Our annual camp serves as a vital lifeline for local medical facilities, bringing together hundreds of donors in a mission of shared humanity.`
- `24/7 Rapid response medical transportation for the community.`
- `Celebrating our rich heritage through vibrant cultural events and festivals that bring the community together in joy and devotion.`
- `Daily food distribution and communal feasts serving the community during festive days.`
- `দুর্গা , আমরা আর আপনারা`
- `Location`
- `Byabattarhat,`
- `The Sangha`
- `Direct Line`
- `+91 98765 43210`
- `Electronic Mail`
- `CONTACT@SARADSANGHA.ORG`
- `Reach out for emergency services, donations, or membership inquiries. Our doors are always open for the community.`

### Micro-copy Labels & Metadata
- `keyboard\_arrow\_down`
- `4`
- `The Heritage Legacy`
- `emergency`
- `Emergency Contact`
- `ambulance`
- `Learn More —`
- `festival`
- `soup\_kitchen`
- `Come to our Puja Pandal every year`
- `Purba Medinipur, WB`
- `Sharad Sangha`

---

## PAGE: ABOUT.HTML
- 📄 **Page Title**: `About Us - Sharad Sangha`
- 🔗 **Source File**: `[about.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/about.html)`

### Headings
- **H1**: `The Story of`
- **H2**: `Our Speakers`
- **H4**: `Sukumar Bera`
- **H4**: `Sankha Churd Bera`
- **H3**: `Tradition`
- **H3**: `Community`
- **H3**: `Service`
- **H2**: `80 Years of Devotion`
- **H3**: `1986`
- **H4**: `Journey Begins`
- **H3**: `1992`
- **H4**: `Anna Seva Milestone`
- **H3**: `1995`
- **H4**: `Official Permission`
- **H3**: `1996`
- **H4**: `Concrete Building`
- **H3**: `1999`
- **H4**: `Official Registration`
- **H3**: `2004`
- **H4**: `Guide Map Inclusion`
- **H3**: `2009`
- **H4**: `Notable Members`
- **H3**: `2012`
- **H4**: `Ambulance Service Starts`
- **H3**: `2016`
- **H4**: `Parir Desh Theme`
- **H3**: `2020`
- **H4**: `Covid-19 Response`

### Content & Paragraphs
- `Since 1948`
- `For eight decades, Byabattarhat Sharad Sangha has stood as a beacon of cultural heritage, community service, and spiritual celebration in the heart of Purba Medinipur.`
- `Voices of Inspiration`
- `"Sharad Sangh is not just a name or an organization. It is an emotion for us. Every part of it is filled with devotion for the Maa Durga."`
- `"Our annual Durga Puja is not just a festival; it is a sacred bond that unites hearts and souls. I am immensely proud of our team's relentless effort in making our Sangha a warm, welcoming home for all devotees and a center of local cultural excellence."`
- `President`
- `"We believe that devotion manifests beautifully in serving humanity. Our emergency health service, blood donation camps, and active social outreach are testimonies to this spirit. Together, we are building a stronger and more compassionate Poribar."`
- `Secretary`
- `We honor the ancient traditions of Durga Puja, maintaining the authenticity and spiritual sanctity of the rituals while embracing thousands of devotees every year.`
- `Our Sangha is a family. Through annual gatherings, cultural programs, and social events, we build strong, lasting bonds among all our members and visitors.`
- `Beyond festivities, we are dedicated to social welfare. From blood donation camps to free emergency ambulances, service to humanity is our core principle.`
- `Our historic journey begins. With great devotion, we organize our first universal Durga Puja festival.`
- `This year is a milestone in our Anna Seva. Around 4,400 people participated in the Anna Seva for Mother's Bhog.`
- `We, along with a few clubs in the undivided Midnapore district, received permission from the Midnapore district administration to organize Durga Puja.`
- `This year the club's own concrete building was built.`
- `We are registered as a social organization under the Society Act, Government of West Bengal.`
- `For the first time, our Durga Puja has been included in the Puja Guide Map published by Purba Medinipur District Police.`
- `The then MP from Tamluk, currently the Honorable Chief Minister of West Bengal, Shri Suvendu Adhikari, accepted the life membership of our organization.`
- `Our ambulance service was started on the recommendation of the honorable MP of Tamluk and a life member of our organization, Shri Suvendu Adhikari, with the financial support of MCPI Haldia (CRS Fund).`
- `Our first themed pandal made of cotton - 'Parir Desh'. The biggest crowd ever gathered at our pandal. The puja was inaugurated by the Honorable District Magistrate and Superintendent of Police. Purba Medinipur.`
- `A new challenge was before us, we jumped into human service. We distributed adequate quantities of masks and sanitizers. We even distributed some PPE kits.`

### Micro-copy Labels & Metadata
- `Our 'Poribar'`
- `“`
- `S`
- `temple\_hindu`
- `groups`
- `favorite`
- `1986`
- `1992`
- `1995`
- `1996`
- `1999`
- `2004`
- `2009`
- `2012`
- `2016`
- `2020`

---

## PAGE: COMMITTEE.HTML
- 📄 **Page Title**: `Committee - Sharad Sangha`
- 🔗 **Source File**: `[committee.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/committee.html)`

### Headings
- **H1**: `The`
- **H2**: `Executive Committee`
- **H3**: `Suvendu Adhikari`
- **H3**: `Sukumar Bera`
- **H3**: `Amit Das`
- **H3**: `Ashim Das`
- **H3**: `Sankha Churd Bera`
- **H3**: `Sandip Metiya`
- **H3**: `Chandan Kr. Dinda`
- **H3**: `Ramakrishna Adhikari`
- **H3**: `Biman Hazra`
- **H2**: `Council Members`
- **H4**: `Dipnarayan Roy`
- **H4**: `Tapan Maity`
- **H4**: `Uttam Sahoo`
- **H4**: `Swarup Pattanayak`
- **H4**: `Susanta Ghara`
- **H4**: `Mahitosh Maji`
- **H4**: `Sandip Chakrabartty`
- **H4**: `Debaprasad Mondal`
- **H4**: `Debadyuti Maity`

### Content & Paragraphs
- `Leadership`
- `Meet the dedicated individuals who volunteer their time and energy to preserve our traditions and serve the community.`
- `Chief Advisor & Lifetime Member`
- `President`
- `Vice-President`
- `Vice-President`
- `Secretary`
- `Asst. Secretary`
- `Treasurer`
- `Cashier`
- `Art Coordinator & It In Charge`
- `Council Member`
- `Council Member`
- `Council Member`
- `Council Member`
- `Council Member`
- `Council Member`
- `Council Member`
- `Council Member`
- `Council Member`

### Micro-copy Labels & Metadata
- `Committee`
- `A`
- `P`
- `VP`
- `S`
- `AS`
- `T`
- `C`
- `I`

---

## PAGE: CONTACT_US.HTML
- 📄 **Page Title**: `Contact Us - Sharad Sangha`
- 🔗 **Source File**: `[contact_us.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/contact_us.html)`

### Headings
- **H1**: `Contact`
- **H3**: `Byabattarhat Sharad Sangha`
- **H3**: `Byabattarhat,`
- **H3**: `Send a Message`

### Content & Paragraphs
- `Get in Touch`
- `Whether you have questions about our upcoming events, wish to participate, or need emergency assistance, we are always here for you.`
- `The Sangha`
- `Direct Line`
- `Electronic Mail`
- `Location`

### Micro-copy Labels & Metadata
- `Us`
- `business`
- `call`
- `+91 98765 43210`
- `mail`
- `CONTACT@SARADSANGHA.ORG`
- `location\_on`
- `Purba Medinipur, WB`
- `Full Name`
- `Email Address`
- `Message`
- `Send Message`

---

## PAGE: GALLERY.HTML
- 📄 **Page Title**: `Gallery - Sharad Sangha`
- 🔗 **Source File**: `[gallery.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/gallery.html)`

### Headings
- **H1**: `Our`
- **H3**: `Lifesaving Donation Camp`
- **H3**: `Volunteer Support`
- **H3**: `Donor Registration`
- **H3**: `Active Participation`
- **H3**: `Noble Contribution`
- **H3**: `Care and Support`
- **H3**: `Serving Humanity`
- **H3**: `Voluntary Donor`
- **H3**: `Community Unity`
- **H3**: `Inspirational Act`
- **H3**: `Appreciation & Honor`
- **H3**: `Title`

### Content & Paragraphs
- `Visual Journey`
- `A visual archive of 80 years of devotion, artistic excellence, and vibrant community celebrations at Byabattarhat Sharad Sangha.`

### Micro-copy Labels & Metadata
- `Heritage`
- `In Pictures`
- `Blood Donation`
- `Our Pujo Diary`
- `Our Golden Memories`
- `>`
- `refresh`
- `Load More Memories`
- `close`
- `chevron\_left`
- `chevron\_right`
- `Category`

---

## PAGE: LEGAL/PRIVACY_POLICY.HTML
- 📄 **Page Title**: `Privacy Policy - Sharad Sangha`
- 🔗 **Source File**: `[legal\privacy_policy.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/legal/privacy_policy.html)`

### Headings
- **H1**: `Privacy`
- **H2**: `1. Introduction`
- **H2**: `2. Information We Collect`
- **H2**: `3. How We Use Your Information`
- **H2**: `4. Contact Us`

### Content & Paragraphs
- `Legal`
- `We value your privacy. Learn how Byabattarhat Sharad Sangha handles and protects your data.`
- `Welcome to the Byabattarhat Sharad Sangha website. We are committed to protecting your personal information and your right to privacy. If you have any questions or concerns about this privacy notice or our practices with regard to your personal information, please contact us at CONTACT@SARADSANGHA.ORG.`
- `We collect personal information that you voluntarily provide to us when you express an interest in obtaining information about us or our products and services, when you participate in activities on the website, or otherwise when you contact us.`
- `We use personal information collected via our website for a variety of business purposes described below. We process your personal information for these purposes in reliance on our legitimate business interests, in order to enter into or perform a contract with you, with your consent, and/or for compliance with our legal obligations.`
- `If you have questions or comments about this notice, you may email us at CONTACT@SARADSANGHA.ORG or by post to:`

### Micro-copy Labels & Metadata
- `Policy`
- `Personal Data:`
- `Names, phone numbers, email addresses, mailing addresses.`
- `Usage Data:`
- `We may automatically collect diagnostic, usage, and performance information when you access our website.`
- `Byabattarhat Sharad Sangha`
- `Byabattarhat, Purba Medinipur, WB`
- `India`

---

## PAGE: PROGRAMS/ANNO_SEVA.HTML
- 📄 **Page Title**: `Anno Seva - Sharad Sangha`
- 🔗 **Source File**: `[programs\anno_seva.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/programs/anno_seva.html)`

### Headings
- **H1**: `Anno`
- **H2**: `"সেবাই পরম ধর্ম"`

### Content & Paragraphs
- `Daily Food Distribution & Feast`
- `Byabattarhat Sharad Sangha's commitment to community welfare extends to nourishing the soul and body. We host daily food distribution drives and communal feasts for everyone during the festive days.`
- `Our kitchen runs non-stop during the five days of Durga Puja, feeding over 5,000+ devotees daily with traditional, blessed meals (Prasad). Every plate is served with love, care, and devotion.`

### Micro-copy Labels & Metadata
- `soup\_kitchen`
- `Seva`
- `SEVA`
- `favorite`
- `Devotional Community Service`
- `(Service to humanity is the highest form of worship)`

---

## PAGE: PROGRAMS/CULTURAL_PROGRAM.HTML
- 📄 **Page Title**: `Cultural Program - Sharad Sangha`
- 🔗 **Source File**: `[programs\cultural_program.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/programs/cultural_program.html)`

### Headings
- **H1**: `Cultural`
- **H3**: `Title`

### Content & Paragraphs
- `সাংস্কৃতিক উৎসব ও ঐতিহ্য`
- `Celebrating community spirit, artistic expression, and local talent. Experience the vibrant moments of dance, drama, and music performed during the festive season of Durga Puja at Byabattarhat Sharad Sangha.`

### Micro-copy Labels & Metadata
- `festival`
- `Programs`
- `close`
- `chevron\_left`
- `chevron\_right`
- `Category`

---

## PAGE: SERVICES/AMBULANCE_CONTACT.HTML
- 📄 **Page Title**: `Emergency Ambulance - Sharad Sangha`
- 🔗 **Source File**: `[services\ambulance_contact.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/services/ambulance_contact.html)`

### Headings
- **H1**: `Emergency`
- **H2**: `Biswajit Paul`

### Content & Paragraphs
- `Ambulance Support Contact`
- `Byabattarhat Sharad Sangha is dedicated to serving the community in times of need. Reach out directly for 24/7 emergency ambulance support.`
- `Direct Mobile`
- `9734252052`

### Micro-copy Labels & Metadata
- `ambulance`
- `Ambulance`
- `24/7`
- `emergency`
- `24/7 Ambulance Service`
- `Ambulance Coordinator`
- `Emergency Rapid Response Team — Byabattarhat Sharad Sangha`
- `call`

---

## PAGE: SERVICES/BLOOD_DONOR.HTML
- 📄 **Page Title**: `Emergency Blood Donor - Sharad Sangha`
- 🔗 **Source File**: `[services\blood_donor.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/services/blood_donor.html)`

### Headings
- **H1**: `Emergency`
- **H2**: `Sandip Chakrabartty`

### Content & Paragraphs
- `Blood Card & Information Contact`
- `In times of crisis, every drop matters. Our annual camp serves as a vital lifeline for local medical facilities, bringing together hundreds of donors in a mission of shared humanity.`
- `Direct Mobile`
- `9434959315`

### Micro-copy Labels & Metadata
- `bloodtype`
- `Blood Donor`
- `B+`
- `emergency`
- `On Call Response`
- `Executive Council Member & Secretary`
- `Purba Medinipur District Voluntary Blood Donor's Association`
- `call`

---

## PAGE: SERVICES/EMERGENCY_SERVICES.HTML
- 📄 **Page Title**: `Emergency Services - Sharad Sangha`
- 🔗 **Source File**: `[services\emergency_services.html](file:///c:/Users/animeshmanna/Desktop/stitch_the_digital_altar_system/services/emergency_services.html)`

### Headings
- **H1**: `Emergency`
- **H3**: `Emergency Ambulance`
- **H3**: `Blood Donation Bank`
- **H2**: `Need Immediate Assistance?`

### Content & Paragraphs
- `24/7 Support`
- `Byabattarhat Sharad Sangha is dedicated to serving the community in times of need with our round-the-clock emergency assistance programs.`
- `24/7 rapid response medical transportation for the community. Fully equipped with necessary life-support and trained personnel.`
- `Access to a large database of verified blood donors. Connect with emergency blood donors or register yourself to save lives.`
- `Our helpline is active 24/7 for the Byabattarhat community.`

### Micro-copy Labels & Metadata
- `medical\_services`
- `Services`
- `01`
- `ambulance`
- `Contact Ambulance`
- `arrow\_forward`
- `02`
- `bloodtype`
- `Find a Donor`
- `call`
- `+91 98765 43210`

---

